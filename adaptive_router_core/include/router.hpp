...
public:
  ...
  [[nodiscard]] RouteResponse route(const float* embedding_data, size_t embedding_size,
                                    float cost_bias = 0.5f);

  // Convenience overload with Eigen vector
  [[nodiscard]] RouteResponse route(const EmbeddingVector& embedding, float cost_bias = 0.5f);
